...
.. req:: Single-fault tolerance for mission-critical function
   :id: REQ_BMS_000
   :status: TBD
   :satisfied_by: TBD

   No single failure of the battery board (radiation-induced (SEU, SEL, SEFI),
   discrete-component, or interconnect) shall result in permanent loss of (a) the board's
   ability to deliver protected power to any output rail, or (b) the OBC's ability to
   observe pack and rail state. Nor shall a fault external to the board propagate into
   permanent loss of the board's own protective or control elements. Exceptions are
   permitted only for elements whose failure probability over mission life is negligible
   relative to the parts they protect, and only with documented reasoning.

   :Verification: FMEA highlighting the effects of removing each single component. Shorts
      can be verified on FlatSat.

.. req:: Battery pack bus voltage
   :id: REQ_BMS_001
   :status: July 2026
   :satisfied_by: SPEC_BMS_001

   The battery pack shall provide up to 33 W sustained (peak power) for 45 minutes
   (eclipse period) at full capacity.

   :Verification: Calculate the capacity of the cells pre-fab. Verify power draw and time
      with a power resistor network in testing. Temperatures must not exceed 60% of
      manufacturer limits.

.. req:: Autonomous cell protection
   :id: REQ_BMS_002
   :status: TBD
   :satisfied_by: SPEC_BMS_002

   The battery pack shall autonomously balance series cells and inhibit charging above
   4.2 V per cell, discharging below 3.0 V per cell, and charging or discharging outside
   the qualified cell temperature range, without dependence on the battery board
   microcontroller, its firmware, or any command from the OBC.

   :Verification: Using simulated resistor cells, verify protection behavior at each edge
      case.

.. req:: Fault isolation and protection coordination
   :id: REQ_BMS_003
   :status: TBD
   :satisfied_by: SPEC_BMS_003

   The battery board shall interrupt a short-circuit fault on any output rail within
   100 µs and isolate that rail without interrupting power to the remaining rails.
   Protection shall be coordinated such that for any fault downstream of the board's
   output protection, the resettable rail-level device opens before the non-resettable
   pack-level device, and such that the worst-case minimum trip threshold of each
   rail-level device exceeds the maximum allocated load current of that rail including
   inrush.

   :Verification: Use single trigger mode to verify trip time.

.. req:: Launch safety inhibits
   :id: REQ_BMS_004
   :status: TBD
   :satisfied_by: SPEC_BMS_004

   The battery board shall hold all battery output current paths open by at least two
   independent series inhibits, one high-side and one low-side, plus an additional one
   on the Payload power line, and shall release them only after the deployment switches
   indicate ejection and the deployment timer has elapsed. Each inhibit shall be held in
   its open state by passive means requiring no powered logic.

   :Verification: Power rails should only allow current to flow iff all 3 inhibits are
      switched on.

.. req:: Telemetry availability under single failure
   :id: REQ_BMS_005
   :status: July 2026
   :satisfied_by: SPEC_BMS_005

   The battery board shall make pack state of charge, pack and per-rail voltage and
   current, and pack temperature available to the OBC through at least two independent,
   non-common-cause paths, such that no single failure of a controller, bus interface, or
   shared arbitration element removes visibility into any of these quantities.

   :Verification: Test data transmission and reception on both methods individually.

.. req:: Cell thermal isolation from board-generated heat
   :id: REQ_BMS_006
   :status: TBD
   :satisfied_by: TBD

   Heat generated by any component on the battery board shall not raise cell temperature
   above the level that reduces the cell's thermal runaway margin, under worst-case
   continuous load in vacuum.

   :Verification: TBD. Likely with testing and derated temperature thresholds.

.. req:: Component survival across mission thermal environment
   :id: REQ_BMS_007
   :status: TBD
   :satisfied_by: TBD

   Every component on the battery board shall remain within its qualified operating
   temperature range across the full eclipse-to-sunlit thermal cycle of the mission
   orbit, at worst-case self-heating under peak continuous load for 5 seconds.

   :Verification: TBD. Likely with testing and derated temperature thresholds. Component
      grade should be at automotive or higher.
